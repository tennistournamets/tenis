-- Fresh installation only. Existing databases use supabase/upgrades/.
-- Generated from the verified step 9 canonical schema; see database-release.json.
do $$ begin
  if to_regclass('public.tournaments') is not null
     or to_regclass('public.organizations') is not null then
    raise exception 'Baseline requires a fresh database; use the documented upgrade procedure';
  end if;
end $$;
